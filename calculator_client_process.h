#include "header.h"
#define CALPORT_PROESS  8999	//计算器的服务器端口号
#define BUFFER_SIZE 1024	//读写信息缓冲区长度

void deal_num1(GtkButton *button, gpointer data)
{
	...
	{	
		...
		servaddr.sin_port = htons(CALPORT_PROESS);  /* 端口号，htons改变字节序 */
...
void calclient_process(int argc,char *argv[])
 {
	 //1.gtk环境初始化
	 gtk_init(&argc, &argv);
	 ...
	 //7.注册信号函数，把entry传给回调函数deal_num1()
	 g_signal_connect(button0, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button1, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button2, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button3, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button4, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button5, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button6, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button7, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button8, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button9, "pressed", G_CALLBACK(deal_num1), entry);
 
	 g_signal_connect(button_add, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button_mul, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button_div, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button_minus, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button_equal, "pressed", G_CALLBACK(deal_num1), entry);
	 g_signal_connect(button_delete, "pressed", G_CALLBACK(deal_num1), entry);
